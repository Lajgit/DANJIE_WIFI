...
class Client:
    ...
    def request(self, command, payload=b"", timeout=2.0, retries=3):
        ...
        raise OtaError(f"命令0x{command:02X}无应答")
...
def connect_bootloader(port, message_id):
    ...
    port.reset_input_buffer()
    try:
        return client, client.request(OTA_HELLO, timeout=0.5, retries=1)
    except OtaError:
...
def upgrade(port, path, target_version, message_id, chunk_size):
    ...
    try:
        ...
        print(f"当前APP版本：{version}（0x{version:08X}）")
    except OtaError:
    ...
    try:
        ...
        print("  INSTALL无应答，继续检查APP版本：", exc)
    ...
    while time.monotonic() < deadline:
        try:
            ...
            print(f"升级后APP版本：{version}（0x{version:08X}）")
    ...
    try:
        with open_port(args.port, args.baud) as port:
            if args.command == "version":
                ...
                print(f"弹界APP版本：{version}（0x{version:08X}）")
            elif args.command == "bootloader":
                _, reply = connect_bootloader(port, args.message_id)
